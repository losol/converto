...
import { closeBrowser, HTMLToPDFService } from "./pdfService.js";
...
async function validatePdfRequest(
	...
	if (html && url) {
		return reply.status(422).send({
			error: "Cannot specify both 'html' and 'url'. Please provide only one.",
		});
	}
	if (!html && !url) {
		return reply
			.status(422)
			.send({ error: "Must specify either 'html' or 'url'." });
	}
	if (url) {
